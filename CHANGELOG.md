...
Фикс оставшихся дырок перевода плагина Bots после его глобального обновления
в конце августа. Новый апстрим и другие места в данный момент не переводятся.

### Added
- Закрыты непереведённые места бандла Bots (`ru-bots-locales.ts`): секция
  пользовательских разделов (`sections`, 18 ключей) и 35 пропущенных функций
  (`noMatchQuery(On)`, `rosterUnavailable`, `allHeldStatus`, `disbandHint/Label/ed`,
  `answerFailed`, `runsDaily/Weekly/...` и др.).
- Проводка русской локали Bots (`import './ru-locales'` в `plugin.tsx`, как у канбана) —
  без неё файл локали копировался, но не подключался.
- Закрыты оставшиеся хардкоды Bots: Advanced-плейсхолдеры, подсказка shared-подписок,
  меню групп, заголовок панели, информативные примеры имени/заголовка нового бота.

### Fixed
- Мусор в `ru-bots-locales.ts`: `savedLocally='local'`, `staleNotice='refresh'`,
  `deleteTitle='Удалить'` (группа) → «Удалить групповой чат?», `disbandAction`-вопрос →
  «Расформировать», `stop`-дубль хинта → «Остановить», `needsYourInput` → полная фраза.
- Дрейф якоря `v122-bots-cron-last-result`: апстрим обернул значение в `routineLastResult(...)`.
- `busyLabel="Deleting…"` / `doneLabel="Deleted"` встречаются дважды в `roster-pane.tsx`
  (удаление бота и группы) — правила с `all: true`.
- Реестр: 720 → **746 правил** (doctor 746/746, GATE 0).
