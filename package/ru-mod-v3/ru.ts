...
export const ru = defineLocale({
  common: {
    ...
    dismiss: 'Закрыть',
    toggleLogs: 'Переключить журналы',
    use: 'Использовать',
  },
  fileMenu: {
  ...
  boot: {
    ...
    failure: {
      title: "Hermes не смог запуститься", description: "Фоновый шлюз не поднялся. Попробуйте одно из решений ниже.",
  ...
  settings: {
    ...
    notifications: {
      title: 'Уведомления',
      intro: 'Нативные уведомления рабочего стола, отдельные от внутриприложных уведомлений. Они локальны для устройства — каждый компьютер хранит свои настройки.',
    ...
    modeOptions: {
      light: { label: 'Светлая', description: 'Светлые поверхности рабочего стола' }, dark: { label: 'Тёмная', description: 'Тёмные поверхности' },
    ...
    model: {
      ...
      moaDesc: 'Настройте именованные пресеты, которые появляются как модели у провайдера Mixture of Agents. Агрегатор — действующая модель.',
      moaEnabled: 'Включён',
      moaNewPreset: 'новый пресет',
      moaPreset: 'Пресет',
      moaTitle: 'Смесь агентов (MoA)',
    },
    providers: {
    ...
    toolsets: {
      ...
      removedMessage: key => `${key} удалён.`, failedSave: key => `Не удалось сохранить ${key}`,
      failedSave: key => `Не удалось сохранить ${key}`,
      failedRemove: key => `Не удалось удалить ${key}`, failedReveal: key => `Не удалось показать ${key}`,
      failedReveal: key => `Failed to reveal ${key}`,
      removeConfirm: key => `Удалить ${key} из .env?`,
      set: 'Установить',
      notSet: 'Не задано',
      selectedTitle: 'Провайдер выбран',
      selectedMessage: provider => `${provider} теперь активен.`, failedSelect: provider => `Не удалось выбрать ${provider}`,
    ...
    billingPage: {
      ...
      manage: 'Управлять',
      noPlansAvailable: 'Сейчас нет тарифов, на которые можно перейти.',
    },
    computerUse: {
      accessibility: 'Специальные возможности',
      accessibilityHint: 'Позволяет cua-driver отправлять клики и нажатия клавиш и читать дерево доступности.',
      approveMessage: 'macOS покажет диалог разрешений от имени CuaDriver. Подтвердите его и вернитесь сюда.',
      ...
      grantsNote: "Разрешения привязаны к собственной идентичности CuaDriver (com.trycua.driver), а не Hermes — поэтому диалог приписывается процессу, который управляет вашим Mac.",
      ...
      platformNoteWin: 'Первый запуск может вызвать запрос Windows SmartScreen для UIAccess-воркера cua-driver — разрешите его.',
      ready: 'Готов',
    ...
    uninstall: {
      ...
      intro: 'Выберите, сколько удалить. Приложение закроется, чтобы завершить процесс; переустановщик можно открыть в любой момент, чтобы вернуться.',
      optionFull: 'Удалить всё',
      optionFullConsequence: 'ВСЁ — Chat GUI, агент Hermes и весь ваш конфиг, чаты, секреты и логи',
      optionFullDesc: 'Удалить приложение, агента и все пользовательские данные — конфиг, чаты, запланированные задачи, секреты, логи.',
      ...
      optionLiteDesc: 'Удалить приложение и агента Hermes, но сохранить конфиг, чаты и секреты для будущей переустановки.',
  ...
  skills: {
    ...
    appliesToNewSessions: name => `${name} применяется к новым сессиям.`, failedToUpdate: name => `Не удалось обновить ${name}`,
  ...
  agents: {
    ...
    files: 'Файлы',
    moreFiles: count => `ещё ${count} файл(ов)`, delegation: index => `Делегирование ${index}`,
    delegation: index => `Делегирование ${index}`,
    workers: count => `${count} воркеров`, workersActive: count => `${count} активны`,
    workersActive: count => `${count} активны`,
    agentsCount: count => `${count} ${ruPlural(count, 'агент', 'агента', 'агентов')}`, activeCount: count => `${count} активны`,
    activeCount: count => `${count} активны`,
    failedCount: count => `${count} с ошибками`, toolsCount: count => `${count} инструментов`,
    ...
    ageNow: 'сейчас',
    ageSeconds: seconds => `${seconds} с назад`, ageMinutes: minutes => `${minutes} мин назад`, ageHours: hours => `${hours} ч назад`, ageDays: days => `${days} дн назад`,
    ageMinutes: minutes => `${minutes} мин назад`, ageHours: hours => `${hours} ч назад`, ageDays: days => `${days} дн назад`,
    ageHours: hours => `${hours} ч назад`,
    ageDays: (days: number) => `${days} дн назад`,
    durationSeconds: seconds => `${seconds} с`, durationMinutes: (minutes, seconds) => `${minutes}:${seconds.toString().padStart(2, '0')} мин`,
  ...
  commandCenter: {
    ...
    nav: {
      newChat: { title: 'Новая сессия', detail: 'Начать новую сессию' }, settings: { title: 'Настройки', detail: 'Открыть настройки' },
    ...
    sectionEntries: {
      sessions: { title: 'Панель сессий', detail: 'Поиск, закрепление и управление сессиями' }, system: { title: 'Системные команды', detail: 'Команды приложения и сервера' },
  ...
  messaging: {
    ...
    disableAria: name => `Отключить ${name}`, platformEnabled: name => `${name} включён`,
    ...
    keyCleared: key => `${key} очищен`, setupUpdated: name => `Настройка ${name} обновлена.`,
    setupUpdated: name => `${name} setup was updated.`,
    failedUpdate: name => `Не удалось обновить ${name}`, failedSave: name => `Не удалось сохранить ${name}`,
  ...
  cron: {
    ...
    weekdaysAt: time => `По будням в ${time}`, everyDayOfWeekAt: (day, time) => `Каждый ${day} в ${time}`,
    everyDayOfWeekAt: (day, time) => `Каждый ${day} в ${time}`,
    monthlyOnDayAt: (dayOfMonth, time) => `Ежемесячно, ${dayOfMonth}-го числа в ${time}`, topOfHour: 'в начале часа',
  ...
  sidebar: {
    ...
    newSessionIn: label => `Новая сессия в ${label}`, showMoreIn: (count, label) => `Показать ещё ${count} в ${label}`,
  ...
  composer: {
    ...
    commandDescs: {
      '/help': 'полный список команд и горячих клавиш', '/clear': 'начать новую сессию',
      '/clear': 'start a new session',
      '/resume': 'возобновить предыдущую сессию', '/details': 'управлять уровнем детализации транскрипта',
    ...
    couldNotPreview: label => `Не удалось предпросмотреть ${label}`, removeAttachment: label => `Удалить вложение: ${label}`,
  ...
  install: {
    ...
    progress: (completed, total) => `Выполнено шагов ${completed} из ${total}`, currentStage: s => `Этап: ${s}`,
  ...
  onboarding: {
    ...
    apiKeyOptions: {
      ...
      openai: { short: 'Модели класса GPT', description: 'Прямой доступ к моделям OpenAI.' }, gemini: { short: 'Модели Google', description: 'Прямой доступ к моделям Gemini.' },
    ...
    startingSignIn: provider => `Запускаем вход для ${provider}…`, verifyingCode: provider => `Проверяем код ${provider}…`,
    verifyingCode: provider => `Verifying your code with ${provider}...`,
    connectedProvider: provider => `${provider} подключён`, connectedPicking: provider => `Выбор провайдера ${provider}…`,
    connectedPicking: provider => `${provider} connected. Picking a default model...`,
    signInFailed: 'Ошибка входа',
    pickDifferentProvider: 'Выбрать другого провайдера',
    signInWith: provider => `Войти через ${provider}`, openedBrowser: provider => `Мы открыли ${provider} в браузере`,
    ...
    externalPending: provider => `${provider} входит через собственный CLI. Выполните эту команду в терминале:`, externalDone: provider => `${provider} подключён`,
  ...
  assistant: {
    thread: {
      ...
      today: time => `Сегодня, ${time}`, yesterday: time => `Вчера, ${time}`,
    ...
    tool: {
      ...
      titleTemplates: {
        ...
        actionTarget: (action, target) => `${action} ${target}`, prefixedDone: (prefix, action) => `${prefix}: ${action} готово`,
  ...
  desktop: {
    ...
    skillCommandsAvailable: count => `${count} команд навыков доступно.`, warningLine: message => `Предупреждение: ${message}`,
    warningLine: message => `предупреждение: ${message}`,
    yoloArmed: 'YOLO активирован для этого чата',
    yoloOff: 'YOLO выкл.',
    yoloSystem: active => `YOLO ${active ? 'вкл' : 'выкл'} для этой сессии`, yoloTitle: 'YOLO',
    yoloTitle: 'YOLO',
