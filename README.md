# 🇷🇺 Hermes Desktop — русский мод (frozen maintenance)
...
**Последний релиз: [v1.2.3.1](https://github.com/upmeister/hermes-desktop-ru/releases/tag/v1.2.3.1) · Hermes 0.21.0 · 3 сентября 2026**  
**Статус: frozen maintenance** — новые переводы не добавляются, только поддержка хардкодов, пока апстрим их не вынесет.

## Зачем этот мод

Официальный русский перевод (`ru.ts`) влит в Hermes Desktop [коммитом `a922dad9`](https://github.com/NousResearch/hermes-agent/commit/a922dad9) (ветка main, v0.21+). Он покрывает каталог i18n: все кнопки, настройки, онбординг и сотни интерфейсных строк.

Однако часть UI остаётся на английском даже при выбранном русском — это **хардкоды** в компонентах, которые ещё не вынесены в `t()` ключи: Боты, канбан, сообщения главного процесса (gateway auth, boot-failure), подписи полей настроек, метаданные плагинов.

Этот мод закрывает эти хардкоды — пока апстрим не переведёт их в систему локалей.

Однако официальный `ru.ts` покрывает **не всё и не всегда удачно**. Например, один из самых заметных примеров: раздел `Skills` (EN `Capabilities` — «Skills, tools, and MCP servers») официально переведён как **«Возможности»** — слово, которое не отражает ни английское название, ни реальное содержимое раздела, и скорее путает русского пользователя, чем помогает (см. конвенции перевода ниже — там примеры более точного авторского перевода терминов).

А целые разделы меню официальный перевод не покрывает вовсе: вкладки и диалоги Bots (создание бота, групповые чаты, разделы), часть канбана, подписи полей настроек (`ru-constants.ts`), сообщения main-процесса. Всё это — зона мода, а не каталога.
...
| | Официальный ru.ts (v0.21+) | Этот мод (хардкоды) |
|---|---|---|
...
- То, что приходит из бэкенда (логи, CLI, traceback, JSON, сообщения ОС), не переводим.
- Информация от внешних источников (провайдеры памяти, сторонние сервисы) тоже не переводится.

Перед записью гоняется **doctor**: косметический пропуск или неоднозначный якорь (часто в Bots) оставит пятно по-английски — установка идёт. Остановка только если пропал файл критичного правила (канбан / connection-registry).
...
| Пара сотен MISSING при установке на свежий Hermes | нормально: Hermes обновляется по несколько раз в день, а мод — нет. Крупное обновление иногда меняет целые паки локалей разом (так слетали 300+ правил в рамках одной минорной версии). Мод при этом почти всегда ставится и работает — критичные поломки он определяет сам. Если видите ошибку перевода или установщика — создавайте issue, так вы поможете и себе, и другим |
