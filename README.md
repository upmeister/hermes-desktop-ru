...
Официальная русская локализация Hermes Agent (`ru.ts`) вошла в `main` 1 сентября 2026 года в коммите [`a922dad9`](https://github.com/NousResearch/hermes-agent/commit/a922dad9) (v0.21+). Она покрывает около 60% интерфейса, но некоторые компоненты по-прежнему содержат **хардкоды** на английском: Bots, Kanban, отдельные группы настроек, сообщения main-процесса и метаданные плагинов.

Этот мод дополняет официальную локализацию, переводя оставшиеся хардкоды и уточняя спорные переводы терминов. Например, `Skills` / `Capabilities` официально переведён как «Возможности», хотя этот раздел посвящён skills, tools и MCP servers.

Подробные **конвенции перевода** и список покрываемых компонентов приведены ниже.

> Нужен Desktop **из исходников** (`hermes desktop` / `git clone`), не portable и не готовый установщик с сайта.
>
> **Windows** — основной путь. **Linux** — установщик прогнан автором (Ubuntu 26). **macOS** — экспериментально, живого прогона нет. Официальный подписанный `.app` / AppImage с сайта **не патчим**.
...
- Устоявшийся термин `Skills` / `Capabilities` → **«Навыки и инструменты»** — официальный перевод «Возможности» не отражает содержание раздела (skills, tools, MCP servers).
