...
export const ru = defineLocale({
  ...
  boot: {
    ...
    failure: {
      ...
      cloudDownDescription:
        'Облачный агент Nous, к которому подключён этот шлюз, возвращает ошибку сервера. Отсюда его нельзя перезапустить — проверьте статус, переключитесь на локальный шлюз или обратитесь в поддержку.',
      cloudDownHint:
        'Кнопки ниже открывают портал Nous (статус инстанса и управление) и наш Discord для поддержки.',
  ...
  settings: {
    ...
    appearance: {
      ...
      terminalFontDesc: 'Выберите установленный шрифт для терминалов Desktop. Nerd Fonts красиво отображают Powerlevel10k и подсказки шелла.',
      ...
      translucencyFrostTitle: 'Матовый',
      translucencyFrost: {
      ...
      vibeHeartsTitle: 'Вайб-сердечки',
      vibeHeartsDesc: 'Парящие сердечки, когда вы говорите спасибо, ily, good bot или отправляете сердечко. Отдельная функция от реакций на сообщения выше.',
    },
    ...
    config: {
      ...
      toolsetsWipeConfirm: 'Удалить все включённые наборы инструментов? Это отключит память, терминал, веб-поиск, делегирование и большинство других инструментов, пока вы их снова не включите.',
    ...
    gateway: {
      ...
      restartHint: 'Питомцам нужен быстрый перезапуск — приложение запущено до появления этой функции. Закройте и снова откройте Hermes, затем вернитесь сюда.',
      ...
      keychainEncryptionDesc: 'Выключено по умолчанию. При включении токены шлюза и учётные данные шифруются системной связкой (Keychain Access, GNOME Keyring или Windows DPAPI) — система может запросить разрешение или пароль. При выключении они хранятся как обычные файлы, доступные только вашему пользователю.',
    ...
    cloudAddHint: 'Совет: вход в Hermes Cloud выше автоматически обнаружит ваших агентов — используйте эту форму только для ручной регистрации известного адреса экземпляра.',
    ...
    managedUpdates: {
      ...
      intro: 'Транзакционное обновление SSH-инсталляций, управляемых Desktop: сессии дренируются, удалённый checkout обновляется, каждый профиль восстанавливается с квитанцией.',
      sshConnection: 'SSH-инсталляция под управлением Desktop',
      update: 'Обновить',
      updating: 'Обновление…',
      progress: 'Дренирование сессий, обновление удалённой инсталляции и восстановление профилей…',
      updated: 'Обновлено',
      ...
      scopesRestored: (profiles: string) => `Восстановлены профили: ${profiles}`,
  ...
  profiles: {
    ...
    remoteOverride: {
      ...
      plainTextOptIn:
        ...
        collisionWarning: 'Шлюз с именем «${label}» уже существует в настройках. Это подключение профиля — отдельное и не изменит его.',
      ...
      removedMessage: '${profile} теперь работает на этом компьютере',
  ...
  cron: {
    ...
    modelImpact: {
      ...
      declined: 'Смена модели отменена — вы отклонили предупреждение о тарифе с обучением на данных.',
    },
    search: 'Поиск',
    loading: 'Загрузка возможностей...',
    states: {
  ...
  updates: {
    ...
    mixedBlockerBody: 'Hermes может закрыть локальные предпросмотры из списка ниже. Остальные процессы нужно закрыть вручную, прежде чем обновление сможет продолжиться.',
    foreignBlockerBody: 'Hermes не может безопасно закрыть эти процессы автоматически. Закройте приложение, терминал или сервис, которому принадлежит каждый из них, затем повторите обновление.',
  ...
  onboarding: {
    ...
    verifyingCode: provider => `Проверяем код для ${provider}...`,
  ...
  preview: {
    ...
    openInBrowser: 'Открыть в браузере',
    openInExternal: 'Открыть во внешнем браузере',
    popIn: 'Вернуть в док',
    popOut: 'Вынести в окно',
  ...
  assistant: {
    thread: {
      ...
      errorOpenDesktopLogs: 'Открыть журналы Desktop',
      errorCopyDiagnostics: 'Копировать сведения об ошибке',
  ...
  desktop: {
    ...
    readOnlyTranscriptBody: 'К этому старому чату пока не подключён ни один бэкенд, поэтому он открыт как транскрипт только для чтения. История сохранена; отправка отключена, пока бэкенд не подхватит его.',
    readOnlyTranscriptSendBlocked: 'Этот чат открыт как транскрипт только для чтения — отправка отключена.',
